...
(when (executable-find "zprintl-0.4.16")
  (global-set-key (kbd "C-x C-h C-z")
		  #'(lambda ()
		      (interactive)
		      (shell-command-on-region
		       (region-beginning) (region-end)
		       "zprintl-0.4.16" (current-buffer) t))))
...
(defvar *stdpackages* '(railscasts-reloaded-theme powerline
					;cyberpunk-2019-theme
						  zerodark-theme
            zprint-mode company
	    markdown-mode elisp-slime-nav geiser paredit slime
	    ;;helm-cider
	    swiper deadgrep ac-cider magit cider
	    web-mode json-mode cyberpunk-theme clojure-mode))
  ...
  (let ((fnt-name "Inconsolata 12"))
    (let ((ft (find-font (font-spec :name fnt-name))))
	  ...
	  (progn
	    ...
	    (set-face-attribute 'default nil :height 120 :weight 'light :slant 'normal))
	(set-face-attribute 'default nil :height 120 :slant 'normal)))))
...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (railscasts-reloaded-theme powerline cyberpunk-2019-theme zprint-mode company markdown-mode elisp-slime-nav geiser paredit slime helm-cider swiper deadgrep ac-cider magit cider web-mode json-mode cyberpunk-theme clojure-mode)))
 '(safe-local-variable-values
   (quote
    ((cider-clojure-cli-global-options . "-J-XX:-OmitStackTraceInFastThrow -A:dev")
